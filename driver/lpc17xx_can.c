...
void CAN_Init(LPC_CAN_TypeDef *CANx, uint32_t baudrate)
{
	...
	temp = temp;//120331 by cq disable warning
...
Status CAN_SendMsg (LPC_CAN_TypeDef *CANx, CAN_MSG_Type *CAN_Msg)
{
	...
	if ((CANx->SR & 0x00000004)>>2)
	{
		...
		if(CAN_Msg->format == EXT_ID_FORMAT)
		{
			...
			CANx->TFI1 |= (1U<<31); //set bit FF//120331 by cq disable warning
		}
		else
		{
			//CANx->TFI1 &= ~(1<<31);
			CANx->TFI1 &= ~(1U<<31);//120331 by cq disable warning
		}
...
//		CANx->TDB1 = *((uint32_t *) &(CAN_Msg->dataB));
	...
	else if((CANx->SR & 0x00000004)>>10)
	{
		...
		if(CAN_Msg->format == EXT_ID_FORMAT)
		{
			...
			CANx->TFI2 |= (1U<<31); //set bit FF//120331 by cq disable warning
		}
		else
		{
			//CANx->TFI2 &= ~(1<<31);
			CANx->TFI2 &= ~(1U<<31);//120331 by cq disable warning
		}
...
//		CANx->TDB2 = *((uint32_t *) &(CAN_Msg->dataB));
	...
	else if ((CANx->SR & 0x00000004)>>18)
	{
		...
		if(CAN_Msg->format == EXT_ID_FORMAT)
		{
			...
			CANx->TFI3 |= (1U<<31); //set bit FF//120331 by cq disable warning
		}
		else
		{
			//CANx->TFI3 &= ~(1<<31);
			CANx->TFI3 &= ~(1U<<31);//120331 by cq disable warning
		}
