...
next_stage:
		...
		case I2C_I2STAT_M_TX_DAT_NACK:
			...
			goto retry;
			//break;//120331 by cq disable warning
...
end_stage:
				// Disable interrupt
				//I2C_IntCmd(I2Cx, 0);
				I2C_IntCmd(I2Cx, (Bool)0);//120331 by cq disable warning
				// Send stop
				I2C_Stop(I2Cx);
	...
	case I2C_I2STAT_S_RX_STA_STO_SLVREC_SLVTRX:
		...
		while(1){
			if (I2Cx->I2CONSET & I2C_I2CONSET_SI){
				// re-Enable interrupt
				//I2C_IntCmd(I2Cx, 1);
				I2C_IntCmd(I2Cx, (Bool)1);//120331 by cq disable warning
				break;
			} else {
				timeout--;
				if (timeout == 0){
		...
		if ((TransferCfg->rx_length != 0) && (TransferCfg->rx_data != NULL)){
			...
			while (TransferCfg->rx_count < TransferCfg->rx_length){
				...
				if (TransferCfg->rx_count < (TransferCfg->rx_length - 1)){
					...
					CodeStatus = I2C_GetByte(I2Cx, &tmp, (Bool)1);//120331 by cq disable warning
					...
					CodeStatus = I2C_GetByte(I2Cx, &tmp, (Bool)0);//120331 by cq disable warning
