...
void FIO_ByteSetDir(uint8_t portNum, uint8_t byteNum, uint8_t bitValue, uint8_t dir)
{
	...
	if(pFIO != NULL) {
		...
		if (dir) {
			...
			if (byteNum <= 3) {//120331 by cq disable warning
		...
		// Input direction
			...
			//if ((byteNum >= 0) && (byteNum <= 3)) {
			if (byteNum <= 3) {
				pFIO->FIODIR[byteNum] &= ~bitValue;//120331 by cq disable warning
			}
		}
	}
}
...
void FIO_ByteSetMask(uint8_t portNum, uint8_t byteNum, uint8_t bitValue, uint8_t maskValue)
{
	...
	if(pFIO != NULL) {
		...
		// Un-mask
			...
			//if ((byteNum >= 0) && (byteNum <= 3)) {
			if (byteNum <= 3) {
				pFIO->FIOMASK[byteNum] &= ~bitValue;//120331 by cq disable warning
			}
		}
	}
}
