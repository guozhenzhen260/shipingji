...
//片选是选通整个LCD屏，串行数据是发送数据，串行时钟是通过控制时钟线来发送数据的
//由于新的大屏其实是由两个屏拼接在一起的，所以他上半屏有上半屏的时钟线，下半屏有下半屏的时钟线，控制这个时钟线就能使用屏了
#define CS_SET()     	LPC_GPIO1->FIODIR |= (1ul << 25); LPC_GPIO1->FIOSET |= (1ul << 25)	   //串行选通
#define CS_CLR()     	LPC_GPIO1->FIODIR |= (1ul << 25); LPC_GPIO1->FIOCLR |= (1ul << 25)
#define SID_SET()    	LPC_GPIO1->FIODIR |= (1ul << 26); LPC_GPIO1->FIOSET |= (1ul << 26)    //串行数据
#define SID_CLR()    	LPC_GPIO1->FIODIR |= (1ul << 26); LPC_GPIO1->FIOCLR |= (1ul << 26) 
#define SCLK_SET()   	LPC_GPIO2->FIODIR |= (1ul << 8);  LPC_GPIO2->FIOSET |= (1ul << 8)    //串行时钟,下半屏
#define SCLK_CLR()   	LPC_GPIO2->FIODIR |= (1ul << 8);  LPC_GPIO2->FIOCLR |= (1ul << 8)
#define SCLK_SET_Up()   LPC_GPIO1->FIODIR |= (1ul << 24); LPC_GPIO1->FIOSET |= (1ul << 24)    //串行时钟,上半屏
...
//由于新的大屏其实是由两个屏拼接在一起的，所以他上半屏有上半屏的时钟线，下半屏有下半屏的时钟线，控制这个时钟线就能使用屏了
//LCD_BL_EN控制背光的亮和暗
#define CS_SET()     	LPC_GPIO3->FIODIR |= (1ul << 25); LPC_GPIO3->FIOSET |= (1ul << 25)	   //串行选通
#define CS_CLR()     	LPC_GPIO3->FIODIR |= (1ul << 25); LPC_GPIO3->FIOCLR |= (1ul << 25)
#define SID_SET()    	LPC_GPIO1->FIODIR |= (1ul << 24); LPC_GPIO1->FIOSET |= (1ul << 24)    //串行数据
#define SID_CLR()    	LPC_GPIO1->FIODIR |= (1ul << 24); LPC_GPIO1->FIOCLR |= (1ul << 24) 
#define SCLK_SET_Up()   LPC_GPIO1->FIODIR |= (1ul << 25); LPC_GPIO1->FIOSET |= (1ul << 25)    //串行时钟,上半屏
#define SCLK_CLR_Up()   LPC_GPIO1->FIODIR |= (1ul << 25); LPC_GPIO1->FIOCLR |= (1ul << 25)
#define SCLK_SET()   	LPC_GPIO1->FIODIR |= (1ul << 26);  LPC_GPIO1->FIOSET |= (1ul << 26)    //串行时钟,下半屏
#define SCLK_CLR()   	LPC_GPIO1->FIODIR |= (1ul << 26);  LPC_GPIO1->FIOCLR |= (1ul << 26)
#define SCLK_BL_SET()   LPC_GPIO2->FIODIR |= (1ul << 8);  LPC_GPIO2->FIOSET |= (1ul << 8)    //背光控制
...
//游戏按键输入P2.7
#define GAMEKey_SetInput() LPC_GPIO2->FIODIR &= ~(1ul << 7)//设为输入模式
#define GAMEKey_Get()      ( ((LPC_GPIO2->FIOPIN >> 7) & 0x01) == 0 )

//120821 by cq ReturnCoinKey
//退币按键输入P0.22
#define ReturnCoinKey_SetInput() LPC_GPIO0->FIODIR &= ~(1ul << 22)//设为输入模式
...
#define MAINTEN_SWITCH()     (LPC_GPIO4->FIOPIN & (1ul << 29))	 //维护开关p4-29
